...
export RESTY_LUAROCKS_VERSION="3.7.0"
export RESTY_OPENSSL_VERSION="1.1.1l"
...
echo RUN;DEBIAN_FRONTEND=noninteractive apt-get update \
...
_2(){
    ...
    curl -fSL -o 3.2.4.zip https://github.com/Taymindis/nginx-link-function/archive/3.2.4.zip && \

    unzip 3.2.4.zip && \
	install -m 644 ./nginx-link-function-3.2.4/src/ngx_link_func_module.h /usr/include/

    
    eval ./configure -j${RESTY_J} ${_RESTY_CONFIG_DEPS} ${RESTY_CONFIG_OPTIONS} ${RESTY_CONFIG_OPTIONS_MORE} ${RESTY_LUAJIT_OPTIONS} \
    --add-module=./nginx-link-function-3.2.4
    make -j${RESTY_J} \
	&& make -j${RESTY_J} install
}
_3(){
...
# cd $HOME_DIRECTORY; cp nginx.conf /usr/local/openresty/nginx/conf/nginx.conf
# cd $HOME_DIRECTORY; cp nginx.vh.default.conf /etc/nginx/conf.d/default.conf
