...
$TTL 300
$ORIGIN ${domain}.
@		SOA ns1.${domain_root}. hostmaster.${domain_root}.(
		20180908000001
		7200
		30M
		3D
...
function Action:updateAction(args)
   local instance = self:getInstance()
    ...
    local ok, err = jobs:add(job)
        -- local _mydata = getIdByType(mytype, id, db[id])
        -- ngx.log(ngx.ERR, "args:" .. json.encode(_mydata))
    end
...
function Action:getAction(args)
    ...
    if not id then
        if parent_id then
            ...
            _data = showFolderDepth(dir_detail, parent_type)
        end
    else
