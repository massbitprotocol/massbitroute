...
local rules = {
    _api_method = [[
        ...
        vhost_traffic_status_filter_by_set_key $api_method ${mbrnodeid}::api_method;
]],
    _node = [[
server {
    listen unix:/tmp/${mbrnodeid}.sock;
    location / {
       ${_api_method()}
        proxy_redirect off;
        proxy_ssl_server_name on;
        proxy_set_header X-Api-Key ${mbrnodekey};
        proxy_set_header Host ${mbrnodeid}.node.mbr.massbitroute.com;
        proxy_pass https://${ip};
