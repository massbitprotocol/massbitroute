...
function Action:registerfixAction(args)
    args.action = nil
...
function Action:registermassbitdotnotofficialAction(args)
    _print(json.encode(args), true)
    args.action = nil
...
function Action:loginAction(args)
    ...
    -- if username ~= "massbit" and username ~= "mison201" and username ~= "duongvu" then
    --     return {
    --         result = false,
    --         err = "Sorry. We only open for some limited account for this phase. <br/> Please contact us on official Discord if you want to test."
    --     }
    -- end
    ...
    local _whitelist = appConfig.whitelist_test
    if _whitelist and _whitelist[username] and _whitelist[username] ~= 1 then
        return {
            result = false,
            err = "Sorry. We only open for some limited account for this phase. <br/> Please contact us on official Discord if you want to test."
        }
    ...
    local session =
        ...
        instance:getRedis(),
        {
            expired = appConfig.sessionExpiredTime
        }
    )
    session:start()
