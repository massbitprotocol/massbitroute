...
export MBR_ENV=latest
#$(git ls-remote --tags --sort='v:refname' https://github.com/massbitprotocol/massbitroute_${TYPE}.git | tail -n1 | cut -d/ -f3)
if [ -z "$MBR_ENV" ]; then
...
git clone --depth 1 $GIT_PUBLIC_URL/massbitprotocol/massbitroute_$TYPE $SITE_ROOT -b ${MBR_ENV}
